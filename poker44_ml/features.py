...
def _hand_features(hand: dict[str, Any]) -> dict[str, float]:
    ...
    for player in players:
        if isinstance(player, dict):
            stack_bb.append(_safe_div(_safe_float(player.get("starting_stack")), 0.02))
    ...
    for action in actions:
        ...
        amount = _safe_float(action.get("normalized_amount_bb"), 0.0)
        before = _safe_div(_safe_float(action.get("pot_before")), 0.02)
        after = _safe_div(_safe_float(action.get("pot_after")), 0.02)

        action_types.append(action_type)
        if actor > 0:
            actor_seats.append(actor)
        street_names.append(street)
        amount_bb.append(max(0.0, amount))
        pot_before.append(max(0.0, before))
        pot_after.append(max(0.0, after))
    ...
    nonzero_amount = sum(1 for value in amount_bb if value > 0.0)
    hero_actions = sum(1 for seat in actor_seats if seat == hero_seat and hero_seat > 0)
    button_actions = sum(
        1 for seat in actor_seats if seat == button_seat and button_seat > 0
    )
    pot_delta = [max(0.0, after - before) for after, before in zip(pot_after, pot_before)]
    monotonic = sum(
        1 for previous, current in zip(pot_after, pot_after[1:]) if current + 1e-9 >= previous
    )
    ...
    return {
        ...
        "schema_starting_stack_iqr_bb": _quantile(stack_bb, 0.75)
        - _quantile(stack_bb, 0.25),
...
def _schema_chunk_features(chunk: list[dict[str, Any]]) -> dict[str, float]:
    if not chunk:
        return {"hand_count": 0.0}

    out: dict[str, float] = {"hand_count": float(len(chunk))}
    per_hand = [_hand_features(hand) for hand in chunk if isinstance(hand, dict)]
    if not per_hand:
        return out
    ...
    for hand, feats in zip(chunk, per_hand):
        actions = hand.get("actions") or []
        action_types = tuple(
            str((action or {}).get("action_type") or "").lower().strip()
            for action in actions
        )
        actor_seq = tuple(
            _safe_int((action or {}).get("actor_seat"), 0)
            for action in actions
            if _safe_int((action or {}).get("actor_seat"), 0) > 0
        )
        street_seq = tuple(
            str((action or {}).get("street") or "").lower().strip()
            for action in actions
        )
        amount_buckets = tuple(
            _amount_bucket(max(0.0, _safe_float((action or {}).get("normalized_amount_bb"))))
            for action in actions
        )
