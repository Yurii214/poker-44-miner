#!/usr/bin/env python3
"""Retune v12 live calibration on recent benchmark dates for R2/R3 rounds."""
...
def eval_recent(model_path: Path, feature_dicts, y, groups, metadata, min_date: str) -> dict:
    ...
    if not mask.any():
        return {}
    # group-level eval like benchmark chunks
    scores = []
    labels = []
    for group_id in np.unique(groups[mask]):
        gmask = mask & (groups == group_id)
        idx = np.flatnonzero(gmask)
        if idx.size == 0:
            continue
        chunk = [feature_dicts[i] for i in idx]
        # rebuild pseudo chunk from feature rows isn't valid - use hybrid path instead
        _ = chunk
    # simpler: per-row via stored artifact simulation
    ...
    live = simulate_regime_live_miner_scores(
        ...
        spread_blend=float(meta.get("live_batch_spread_blend", 0.94)),
        ...
        bot_max_positive_rate=float(meta.get("live_bot_max_positive_rate", 0.24)),
...
def main() -> None:
    ...
    backup_path = REPO_ROOT / "models" / "bot_detector_v1.reference-dualbranch-v12-r1-recall_backup.joblib"
    ...
    version = "reference-dualbranch-v12-r2-calibrated"
    ...
    print(json.dumps({"calibration": cal, "recent_eval_before": eval_recent(source_path, feature_dicts, y, groups, metadata, "2026-06-28")}, indent=2))
    print(json.dumps({"recent_eval_after": eval_recent(out, feature_dicts, y, groups, metadata, "2026-06-28")}, indent=2))
